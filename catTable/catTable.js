...
(function (w) {
    ...
    if (!w.catUI || !w.catUI.version) throw "The catUI must be imported";
    ...
    var table = {
        ...
        init: function (option) {
            //table组件的建造者
            return buildTable(option);
        }
    };
    //将组件注册到catUI里面去
    catUI.register(table);
    ...
    function _table(obj) {
        ...
        if (!obj.thead || obj.thead.__proto__ != Array.prototype || obj.thead.length == 0) {
        ...
        _thshowType = typeof thead[0] == 'string' ? 'String' : 'Object';
        ...
        //是否展示页脚的选择元素
        if (obj['showPageSelect']) {
            if (typeof obj['showPageSelect'] != 'boolean') throw 'showPageSelect must be type of Boolean!';
        ...
        var _element = _targetDiv = doc.querySelector(ele);//组件依附的div
         ...
         * 重新设置初始化param
         */
        _selfFuns.setInitParam = function (p) {
            param = p;
        }
        /**
         * 传入p 表示回到确定的某一页(pageNumber+1为真实的页数),不传表示回到首页
         */
        _selfFuns.reloadTable = function (p) {
            var _index = 0;
         ...
         * 重新设置pageSize
         */
        _selfFuns.setPageSize = function (p) {
            if (!/^[1-9]+[0-9]*$/.test(p)) {
         ...
         * 获取当前的pageNumber
         */
        _selfFuns.getCurPageNum = function () {
            return pageNumber;
        }
         ...
         * 获取当前页的原始数据
         */
        _selfFuns.getData = this.getCurrentData = function () {
            return _currentData;
        }
         ...
         * 向真实的页面DOM中插入表格
         */
        _element.appendChild(_finalDomTree);
        /**
         * 向真实的页面DOM中插入页脚
         */
        if (showFooter) {
            _element.appendChild(_footerDomTree);
        }
        /**
         * tr的点击事件添加
         */
        _clickTrChangeColor.call(_element);
        /**
         * 页脚事件的添加
         */
        if (showFooter) {
             ...
             * 上一页
             */
            _clickPreviousStep.call(_element);
            /**
             * 下一页
             */
            _clickNextStep.call(_element);
            /**
             * 修改input直接跳转到对应页
             */
            _changeInputIndexNum.call(_element);
        }
        /**
         * 如果显示页脚选择，则需要添加对应的事件
         */
        if (showFooter) {
            if (showPageSelect) {
                _clickFooterIndex.call(_element);
            }
        }
